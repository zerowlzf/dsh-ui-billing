...
This package prices a Web session from rates the user owns. Its Host half declares the `ui-billing` fields live and caches two provider reads in them: the DeepSeek account balance and the published price table. Its browser half renders two cost figures in the composer's dock, a cost pill in every completed Turn's tail, and the Billing page on the Plugins page. A route is a `provider/model` pair priced per million tokens in up to two daily windows — cached input, uncached input, output — at the peak rate or the published off-peak one. The balance is the DeepSeek account's.
...
Each completed Turn carries its own cost pill in its tail, through `conversation.chat.turnTail` — the list ui-chat declares for feature contributions before that Turn's action row, whose owner share is the Turn, its closing sequence, and the file opener. It shows `费用 ¥0.42` and opens that Turn's breakdown: one row for the route the Turn's own durable accounting names, charged at the price window in force when the Turn closed. A Turn that also produced files shows both tail entries — the shipped files row and this figure — because the tail is a list, so neither displaces the other. Three cases carry no figure, and the dialog says which one applies:
...
- its accounting names several routes — a Turn that retried on another model, or switched route mid-way — and nothing loaded states which attempt ran on which, so the split cannot be made; the dialog names the routes it could not attribute rather than charging the whole aggregate once per route;
- a route it ran on has no configured rates, which the dialog names as unpriced.

An interrupted Turn keeps its tail too: no closing message means no copy or branch target, but the accounting and the cost pill are what that tail is for. That pill introduces the action row below it, so it follows that row's own reveal: a Turn whose row is shown only on hover hides the pill until then, while an interrupted Turn carries no reveal attribute and keeps its cost resident. A Turn that has not ended yet has no tail — the shipped tail node appears when the Turn closes — so the newest answer carries its cost only once that Turn settles.
...
- **A Turn that ran on several routes shows no cost** — a Turn that retried on another model, or switched route mid-way, carries one aggregate plus the routes that billed it, and nothing loaded states which attempt ran on which: the pill withholds the figure and the dialog names the routes. A Turn whose own accounting names one route is priced exactly, which is every Turn of an ordinary session on one provider.
- **A Turn that spans a price boundary is charged at one band** — the Turn's own close time is the only instant its accounting carries, so a Turn running across 12:00 or 18:00 Beijing time is charged at the band in force when it closed.
...
- The cost figure is a tail contribution (`conversation.chat.turnTail`), the list ui-chat declares for feature contributions before a completed Turn's action row, and a list is what keeps it: the shipped produced-files entry and this figure coexist for the same Turn. The pill reads the `turn-tail` payload alone — its `tokenUsage` and its `time`; no Chat row carries the route an attempt was billed on, which is why a Turn its accounting attributes to several routes is withheld rather than split by guesswork.
